...
window.particlesJS.load = function(tag_id, path_config_json, callback){
  ...
  xhr.send();
	...
	if ( $( '.ha-particle-yes' ).length ) {

		//console.log(window.scope_array);
	...
	function particles_background_script( $scope ) {
		...
		if( typeof particles_style != 'undefined' ) {
			if( element_type == 'column' ) {
				...
				$particle_wrap = $scope.find( '.ha-particle-wrapper' );
				...
				$particle_wrap = $scope.find( '#ha-particle-' + id );
				if( $bg_wrap.next( '#ha-particle-' + id ).length == 0 ) {
					$bg_wrap.after( $particle_wrap );
				}
			}
			$selector = $scope.find( '#ha-particle-' + id );
			...
			$selector = $scope;
			$content = $( '<div class="ha-particle-wrapper" id="ha-particle-' + id + '"></div>' );
